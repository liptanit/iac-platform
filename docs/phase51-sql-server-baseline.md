...
- Installs SQL Server only when `windows_sql_install_source` points to a path
...
Set this in a root-only Ansible environment file or Semaphore secret before
requesting install:
...
source is missing, the default production policy does not fail the rollout; it
records SQL as missing and leaves evidence for the operator.
